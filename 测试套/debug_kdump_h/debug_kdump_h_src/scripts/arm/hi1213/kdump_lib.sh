...
function base_deploy()
{
    ...
    deploy -t $TARGET_NAME -u $APPLYBOARD_USER -f $FIRST_KERNEL_ROOTFS,$FIRST_KERNEL_IMAGE,$FIRST_KERNEL_DTB -a &
    pid=$!
    cd - > /dev/null
}
...
function kexec_load()
{
    kexec_cmd="kexec -p -l ${dst_capture_kernel_path}/${CAPTURE_KERNEL_IMAGE} --initrd=${dst_capture_kernel_path}/${CAPTURE_KERNEL_ROOTFS} --dtb=${dst_capture_kernel_path}/${CAPTURE_KERNEL_DTB} --append='maxcpus=1 console=ttyS0 root=/dev/ram0 rdinit=/sbin/init'"

...
boot_password=huawei123
boot_enter_key=s
boot_keyword=UBBP EFI
boot_line=c
boot_line=HiFE
boot_line=0
boot_line=0
...
function base_deploy()
{
    ...
    echo "PLEASE Make sure $FIRST_KERNEL_IMAGE $FIRST_KERNEL_ROOTFS and $FIRST_KERNEL_DTB is in server_image_dir,like 9.84.1.112:/home/ftp"
...
function kexec_load()
{
    kexec_cmd="kexec --type zImage -p -l ${dst_capture_kernel_path}/${CAPTURE_KERNEL_IMAGE} --initrd=${dst_capture_kernel_path}/${CAPTURE_KERNEL_ROOTFS} --dtb=${dst_capture_kernel_path}/${CAPTURE_KERNEL_DTB} --append='maxcpus=1 console=ttyS0,9600 root=/dev/ram0 rdinit=/sbin/init'"
...
{
    makedumpfile_cmd="${MAKEDUMPFILE_TOOL_DIR}makedumpfile --message-level=31 -c -d 31 /proc/vmcore /tmp/vmcore_my"

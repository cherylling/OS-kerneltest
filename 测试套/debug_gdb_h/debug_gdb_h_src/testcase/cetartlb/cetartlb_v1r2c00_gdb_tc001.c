...
int main(int argc, char **argv)
{
    ...
    tlb_fd = open(argv[1], O_RDONLY);
    if (tlb_fd < 0){
        printf("open %s O_RDONLY error\n", argv[1]);
        return 1;    
    }
...
#ifdef MMAP_64
    mmap_addr = (char *)mmap64((void *)ADDR, length, PROT_NONE, MAP_SHARED|MAP_FIXED, tlb_fd, offset);
#else
    mmap_addr = mmap((void *)ADDR, length, PROT_NONE, MAP_SHARED|MAP_FIXED, tlb_fd, offset);
#endif 

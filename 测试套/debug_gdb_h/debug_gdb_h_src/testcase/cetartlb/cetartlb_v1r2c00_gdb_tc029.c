...
int main(int argc, char **argv)
{
...
#ifdef MMAP_64
    mmap_addr = (char *)mmap64((void *)ADDR, length, PROT_READ|PROT_WRITE, MAP_SHARED|MAP_FIXED, tlb_fd, offset);
#else
    mmap_addr = mmap((void *)ADDR, length, PROT_READ|PROT_WRITE, MAP_SHARED|MAP_FIXED, tlb_fd, offset);
#endif 
    ...
    int ret = mprotect(mmap_addr, length, PROT_READ|PROT_WRITE);
    if (ret < 0){
        printf("mprotect PROT_READ|PROT_WRITE fail unexpected\n");
        close(tlb_fd);
        munmap(mmap_addr, length);
        return 1;
    }

    printf("mprotect PROT_READ|PROT_WRITE pass as expected\n");
    printf("%c \n",mmap_addr[0]);
    
    memcpy(mmap_addr, mcpy_str, strlen(mcpy_str));
    close(tlb_fd);
    munmap(mmap_addr, length);
    printf("%s should get no SIGSEGV\n", argv[0]);
    return 0;
}

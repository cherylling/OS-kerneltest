...
###register kprobes
echo "r:mykprobe $do_fork" > /sys/kernel/debug/tracing/kprobe_events
cat /sys/kernel/debug/tracing/kprobe_events | grep "r:kprobes/mykprobe $do_fork"
if [ $? -ne 0 ];then
	echo "register kprobes fail\n"
	RET=$(($RET+1))
fi
echo 'r:mykprobe1 do_sys_open' >> /sys/kernel/debug/tracing/kprobe_events
cat /sys/kernel/debug/tracing/kprobe_events | grep "r:kprobes/mykprobe1 do_sys_open"
if [ $? -ne 0 ];then
        echo "register kprobes fail\n"
        RET=$(($RET+2))
fi

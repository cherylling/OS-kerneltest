...
###register kprobes
echo 'p:mykprobe do_sys_open mode=+4($stack)' > /sys/kernel/debug/tracing/kprobe_events
...
if [ "$(uname -r)" \< "4.2" ];then
	echo 'r:mykprobe1 do_fork $retval' >> /sys/kernel/debug/tracing/kprobe_events
	cat /sys/kernel/debug/tracing/kprobe_events | grep "r:kprobes/mykprobe1 do_fork"
else
	echo 'r:mykprobe1 _do_fork $retval' >> /sys/kernel/debug/tracing/kprobe_events
	cat /sys/kernel/debug/tracing/kprobe_events | grep "r:kprobes/mykprobe1 _do_fork"
fi
if [ $? -ne 0 ];then
        echo "register kprobes fail\n"
        RET=$(($RET+2))

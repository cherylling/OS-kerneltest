...
avg=0
max=0
new_image="1layering_1000m"
mkdir -p ${CURRENT_DIR}/build

function docker_build_1layering_1000M()
{
    do_init
    if [ $? -ne 0 ];then
    ...
    dd if=/dev/zero of=${CURRENT_DIR}/build/${new_image} bs=1M count=1000
...
docker_build_1layering_1000M

mkdir -p /tmp/cloudran
cp $tmp_file /tmp/cloudran/build_1layering_1000M
rm -rf $tmp_file

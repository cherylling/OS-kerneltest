...
function docker_build_2layering()
{
    do_init
    if [ $? -ne 0 ];then
    ...
    cat > ${CURRENT_DIR}/build/Dockerfile << EOF
FROM layering_1
COPY $new_image /root/layering_2
EOF
    ...
    docker rmi layering_1 > /dev/null 2>&1
    do_uninit
    
}

docker_build_2layering

mkdir -p /tmp/cloudran
cp $tmp_file /tmp/cloudran/build_2layering
rm -rf $tmp_file

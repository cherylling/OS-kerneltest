...
function docker_boottime_single_idle()
{
    do_init
    if [ $? -ne 0 ];then
    ...
    do                      
        t1=`date +%s.%N`
        t2=`docker run -ti --net=none --rm ${ubuntu_image} date +%s.%N`
...
docker_boottime_single_idle

mkdir -p /tmp/cloudran
cp $tmp_file /tmp/cloudran/boottime_single
rm -rf $tmp_file

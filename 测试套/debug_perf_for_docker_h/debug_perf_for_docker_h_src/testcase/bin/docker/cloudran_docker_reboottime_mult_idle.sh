...
function docker_reboottime_parallel_idle()
{

    t1=`date +%s.%N`
    for var in ${conid[@]}
    do
        docker restart $var > /dev/null 2>&1 &
    done
    wait
    t2=`date +%s.%N`
...
function docker_reboottime_mult_idle()
{
    do_init
    if [ $? -ne 0 ];then
    ...
    for n in $(seq 0 19)
    do
        conid[$n]=`docker run -itd --net=none ${ubuntu_image}` > /dev/null
    done


    for i in $(seq 1 100)
    do
        docker_reboottime_parallel_idle
    done
...
docker_reboottime_mult_idle

mkdir -p /tmp/cloudran
cp $tmp_file /tmp/cloudran/reboottime_mult
rm -rf $tmp_file

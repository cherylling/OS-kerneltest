...
function set_env()
{
    ...
    #create dockers
    docker_name="port_test"
    docker run -tid --name ${docker_name}  -p 10001:10001 ${ubuntu_image} /bin/sh
    if [ $? -ne 0 ];then
...
function do_test()
{
    ...
    do
        t1=`date +%s.%N`
        port_map=`docker port ${docker_name}`
        t2=`date +%s.%N` 
        echo ${port_map} | grep 10001 || return 1
...
cp $tmp_file /tmp/cloudran/port_latency

...
function docker_build_4layering()
{
    do_init
    if [ $? -ne 0 ];then
    ...
    build_with_no_parent layering_3 layering_2 ${new_image} layering_3
    if [ $? -ne 0 ];then
        echo "docker build --no-parent 3layering: FAIL"
        exit $RET 
    fi
    ...
    cat > ${CURRENT_DIR}/build/Dockerfile << EOF
FROM layering_3
COPY $new_image /root/layering_4
EOF
    ...
    docker rmi layering_3 layering_2 layering_1 > /dev/null 2>&1
    do_uninit
    
}

docker_build_4layering

mkdir -p /tmp/cloudran
cp $tmp_file /tmp/cloudran/build_4layering
rm -rf $tmp_file

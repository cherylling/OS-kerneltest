...
avg=0
max=0
new_image="1000m_layering_image"
...
EOF
    ...
    docker build --no-parent -t ${new_image} -f ${CURRENT_DIR}/Dockerfile.1000m ${CURRENT_DIR} > /dev/null
    if [ $? -ne 0 ];then
        echo "docker build 1000m_layering_image: FAIL"
        RET=1
        exit $RET 
    fi
...
function docker_save_layering_image_1000M()
{
    do_init
    if [ $? -ne 0 ];then
...
docker_save_layering_image_1000M

mkdir -p /tmp/cloudran
cp $tmp_file /tmp/cloudran/save_layering_image_1000M
rm -rf $tmp_file

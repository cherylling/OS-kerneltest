...
avg=0
max=0
new_image="100m_layering_image"

function build_new_image()
{
    dd if=/dev/zero of=${CURRENT_DIR}/100m_file bs=1M count=100

    cat > ${CURRENT_DIR}/Dockerfile.100m << EOF
FROM $ubuntu_image
COPY 100m_file /root
EOF

    docker build --no-parent -t ${new_image} -f ${CURRENT_DIR}/Dockerfile.100m ${CURRENT_DIR} > /dev/null
    if [ $? -ne 0 ];then
        echo "docker build 100m_layering_image: FAIL"
        RET=1
        exit $RET 
    fi

    rm -rf  ${CURRENT_DIR}/Dockerfile.100m ${CURRENT_DIR}/100m_file > /dev/null

}

function docker_save_layering_image_100M()
{
    do_init
    if [ $? -ne 0 ];then
...
docker_save_layering_image_100M

mkdir -p /tmp/cloudran
cp $tmp_file /tmp/cloudran/save_layering_image_100M
rm -rf $tmp_file

...
function set_env()
{
    ...
    #create dockers
    docker_name="kill_test"
    docker run -tid --name ${docker_name} ${ubuntu_image} "/bin/sh"
    if [ $? -ne 0 ];then
...
function do_test()
{
    ...
    do
        ...
        #start docker
        docker ps -a | grep ${docker_name} | grep Exited
        if [ $? -ne 0 ];then
            echo "${i}th kill docker[${docker_name}]:FAIL"
            return 1
        fi
...
cp $tmp_file /tmp/cloudran/kill_latency

...
function set_env()
{
    ...
    #create dockers
    docker run -tid --net=none --name rename_test ${ubuntu_image} /bin/sh
    if [ $? -ne 0 ];then
...
function do_test()
{
    ...
    do
        old_name=`docker ps | grep rename_test | awk '{print $NF}'`
        echo ${old_name} | grep rename_test 
        if [ $? -ne 0 ];then
            echo "Get docker name:FAIL"
            return 1
        fi
        new_name=rename_test_$i
        t1=`date +%s.%N`
...
cp $tmp_file /tmp/cloudran/rename_latency

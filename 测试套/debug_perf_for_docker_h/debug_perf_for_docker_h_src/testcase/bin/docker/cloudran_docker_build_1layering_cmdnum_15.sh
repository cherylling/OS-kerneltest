...
avg=0
max=0
new_image="1layering_cmdnum_15"
mkdir -p ${CURRENT_DIR}/build

function docker_build_1layering_cmdnum_15()
{
    do_init
    if [ $? -ne 0 ];then
...
docker_build_1layering_cmdnum_15

mkdir -p /tmp/cloudran
cp $tmp_file /tmp/cloudran/build_1layering_cmdnum_15
rm -rf $tmp_file

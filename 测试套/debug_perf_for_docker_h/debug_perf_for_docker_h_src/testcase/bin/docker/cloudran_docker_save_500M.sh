...
avg=0
max=0
new_image="400m_image"

function build_new_image()
{
    dd if=/dev/zero of=${CURRENT_DIR}/400m_file bs=1M count=400

    cat > ${CURRENT_DIR}/Dockerfile.400m << EOF
FROM $ubuntu_image
COPY 400m_file /root
EOF

    docker build -t ${new_image} -f ${CURRENT_DIR}/Dockerfile.400m ${CURRENT_DIR} > /dev/null
    if [ $? -ne 0 ];then
        echo "docker build 400m_image: FAIL"
        RET=1
        exit $RET 
    fi

    rm -rf  ${CURRENT_DIR}/Dockerfile.400m ${CURRENT_DIR}/400m_file > /dev/null

}

function docker_save_500M()
{
    do_init
    if [ $? -ne 0 ];then
...
cp $tmp_file /tmp/cloudran/save_500M
rm -rf $tmp_file

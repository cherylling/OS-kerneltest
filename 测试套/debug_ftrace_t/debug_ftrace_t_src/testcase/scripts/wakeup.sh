...
init()
{
   ...
   grep wakeup ${TRACING_PATH}/available_tracers
   if [ $? -ne 0 ];then
       echo "no wakeup in ${TRACING_PATH}/available_tracers"
       exit 1
   fi
...
do_test()
{
    ...
    echo > ${TRACING_PATH}/trace
    echo wakeup > ${TRACING_PATH}/current_tracer
    echo 1 > ${TRACING_PATH}/tracing_on
    ...
    if [ $TRACER_NAME != "wakeup" ];then
        echo "wakeup tracer test fail"
        clean
        exit 1
    fi

...
int my_nmi_handler1(void)
{
	dump_stack();
	printk("register my nmi handler1 on cpu%d: dump kernel stack.\n",cpu_num);
	return 0;
}

int my_nmi_handler2(void)
{
        dump_stack();
        printk("register my nmi handler2 on cpu%d: dump kernel stack.\n",cpu_num);
        return 0;
}
...
{
	int ret;
	ret=register_nmi_handler(cpu_num,my_nmi_handler1);
	if (ret != 0)
	{
		printk("register nmi handler1 failed!\n");
		return -1;
	}

	ret=register_nmi_handler(cpu_num,my_nmi_handler2);
        if (ret != 0)
        {
                printk("register nmi handler2 failed!\n");
                return -1;
        }

...
#   4. 获取进程线程tid
#   5. 使用perf record -t TID -S  obj.o,十进制地址/十六进制,len 对进程采集数据
...
dotest(){
	...
	start_addr_10=$((start_addr_16))
	size_10=`wc -c ./tc_obj_01.o | awk '{print $1}'`

	# 1. 起始地址
	perf record -t $TID -S $obj_name,$start_addr_10,$size_10 &
	perf_pid=$!
	...
	if [ $RET -eq 0 ];then
		echo "[PASS] -S $obj_name,$start_addr_10,$size_10 pass !"
	fi
}

...
# 
# Description:  perf -S功能，obj_path后缀为.a, 相对路径
#   1. 启动无符号的（strip）二进制可执行文件启动进程
#   2. 进程中调用另一模块的函数wrap_calc()
#   3. 在wrap_calc中调用另一模块的其他三个函数func_a,func_b,func_c
#   4. 获取进程线程tid
#   5. 使用perf record -t TID -S  modulename.a,start_addr,len 对进程采集数据
...
dotest(){
    obj_name="./tc_obj_01.a"
	[ -f $obj_name ] && rm -rf $obj_name
	cp ./tc_obj_01.o $obj_name

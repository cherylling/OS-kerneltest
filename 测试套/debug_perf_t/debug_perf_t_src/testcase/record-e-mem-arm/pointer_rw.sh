...
touch $step2
perf record -o $perf_data_path -e mem:$addr:rw -p `pidof $func`

#perf record -e mem:0x$addr:rw $opt $func
#val=`perf report | grep memval | wc -l`
ret=`perf report -i $perf_data_path | grep -E "Samples:[[:blank:]]2"`
...
#read check
#valr=`perf report | grep memval | head -n 1 | awk -F ":" '{print $2}' | awk -F "x" '{print $2}'`
#write check
#valw=`perf report | grep memval | tail -n 1 | awk -F ":" '{print $2}' | awk -F "x" '{print $2}'`
:<<COMMENT
if [ $valr = $valw ];then
	{
		echo "FAIL:pointer value write error"
		exit 1
	}
fi
COMMENT

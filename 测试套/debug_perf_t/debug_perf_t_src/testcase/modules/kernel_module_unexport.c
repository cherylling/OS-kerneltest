...
ssize_t planck_free_read_unexport(struct file *file,char __user *buf,size_t count,loff_t *ppos)
{
    char buffer[15]={0};
    int ret=func_count();
    printk("the func_count ret is %d\n",ret);
    snprintf(buffer,sizeof(buffer),"0x%1x\n",free_enable);
    return simple_read_from_buffer(buf,count,ppos,&buffer,strlen(buffer));
}

ssize_t planck_free_write_unexport(struct file *file,const char __user *buf,size_t count,loff_t *ppos)
{
    char buffer[15],*end;
    unsigned long val=0;
...
static struct file_operations f_ops={
    .write=planck_free_write_unexport,
    .read=planck_free_read_unexport,
};

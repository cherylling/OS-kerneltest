...
static int __init jprobe_init(void)
{
...
#endif
	jp2.entry = (kprobe_opcode_t *)jprobe_do_fork;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,2,0)
        jp2.kp.addr = (kprobe_opcode_t *)kallsyms_lookup_name("_do_fork");
#else
        jp2.kp.addr = (kprobe_opcode_t *)kallsyms_lookup_name("do_fork");
#endif

	ret=register_jprobe(&jp);
	if (ret < 0) {
                printk(KERN_DEBUG "register_jprobe failed\n");
                return -1;
        }
	...
	if (ret < 0)
		...
		unregister_jprobe(&jp);
                printk(KERN_DEBUG "register_jprobe2 fail\n");
                return -1;
        }
	return 0;	
}

...
//            do_child(subchild);
    ...
    else
    {
        ...
        if(ptrace_ret != 0)
        {
            printf("ptrace PTRACE_SETOPTIONS  PTRACE_O_TRACEEXIT error %d \n", errno);
            ptrace(PTRACE_CONT, child, NULL, NULL);
            exit(1);
        }
        printf("ptrace PTRACE_SETOPTIONS PTRACE_O_TRACEEXIT success\n");
...
//        ptrace_ret = ptrace(PTRACE_GETEVENTMSG, atoi(rbuf), NULL, &msg);
    ...
    if(flag == 1)
    {
        printf("ptrace PTRACE_SETOPTIONS PTRACE_O_TRACEEXIT error \n");

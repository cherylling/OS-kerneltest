...
//            do_child(subchild);
    ...
    else
    {
        ...
        ptrace_ret = ptrace(PTRACE_SETOPTIONS, child, NULL, PTRACE_O_TRACESECCOMP);
        if(ptrace_ret != 0)
        {
            printf("ptrace PTRACE_SETOPTIONS  PTRACE_O_TRACESECCOMP error %d \n", errno);
            ptrace(PTRACE_CONT, child, NULL, NULL);
            exit(1);
        }
        printf("ptrace PTRACE_SETOPTIONS PTRACE_O_TRACESECCOMP success\n");
...
//        ptrace_ret = ptrace(PTRACE_GETEVENTMSG, atoi(rbuf), NULL, &msg);
    ...
    if(flag == 1)
    {
        printf("ptrace PTRACE_SETOPTIONS PTRACE_O_TRACESECCOMP error \n");

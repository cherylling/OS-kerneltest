...
//        sleep(1);
    ...
    else
    {
        ...
        //set the eip
        ptrace_ret = ptrace(PTRACE_POKEDATA, child, regs.eip, NULL);
        if (-1 == ptrace_ret)
        {
            printf("PTRACE POKEDATA FAILED! errno %d\n", errno);
            ptrace(PTRACE_CONT, child, NULL, NULL);
            exit(1);
        }  
        ...
        //set the rip
        ptrace_ret = ptrace(PTRACE_POKEDATA, child, regs.rip, NULL);
        if (-1 == ptrace_ret)
        {
            printf("PTRACE POKEDATA FAILED! errno %d\n", errno);
            ptrace(PTRACE_CONT, child, NULL, NULL);
        ...
        printf("regs.regs.r15 success %lx \n", regs.regs.nip);
        //set the nip
        ptrace_ret = ptrace(PTRACE_POKEDATA, child, regs.regs.nip, NULL);
        if (-1 == ptrace_ret)
        {
            printf("PTRACE POKEDATA FAILED! errno %d\n", errno);
            ptrace(PTRACE_CONT, child, NULL, NULL);
        ...
        printf("ptrace PTRACE_GETREGS success %lx \n", regs.regs.uregs[15]);
        //set the uregs[0]
        ptrace_ret = ptrace(PTRACE_POKEDATA, child, regs.regs.uregs[15], NULL);
        if (-1 == ptrace_ret)
        {
            printf("PTRACE POKEDATA FAILED! errno %d\n", errno);
            ptrace(PTRACE_CONT, child, NULL, NULL);
        ...
        printf("ptrace PTRACE_GETREGS success %lx \n", pt_regs[15]);
        //set the uregs[0]
        ptrace_ret = ptrace(PTRACE_POKEDATA, child, pt_regs[15], NULL);
        if (-1 == ptrace_ret)
        {
            printf("PTRACE POKEDATA FAILED! errno %d\n", errno);
            ptrace(PTRACE_CONT, child, NULL, NULL);
        ...
        printf("ptrace PTRACE_POKEDATA success %lx \n", ptrace_ret);

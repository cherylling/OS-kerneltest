...
//            do_child(subchild);
    ...
    else
    {
        ...
        ptrace_ret = ptrace(PTRACE_SETOPTIONS, child, NULL, PTRACE_O_TRACEVFORKDONE);
        if(ptrace_ret != 0)
        {
            printf("ptrace PTRACE_SETOPTIONS  PTRACE_O_TRACEVFORKDONE error %d \n", errno);
            ptrace(PTRACE_CONT, child, NULL, NULL);
            exit(1);
        }
        printf("ptrace PTRACE_SETOPTIONS PTRACE_O_TRACEVFORKDONE success\n");
...
//        ptrace_ret = ptrace(PTRACE_GETEVENTMSG, atoi(rbuf), NULL, &msg);
    ...
    if(flag == 1)
    {
        printf("ptrace PTRACE_SETOPTIONS PTRACE_O_TRACEVFORKDONE error \n");

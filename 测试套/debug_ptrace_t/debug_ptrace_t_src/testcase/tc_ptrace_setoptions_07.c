...
int main()
{
    ...
    int result = SIGTRAP | (PTRACE_EVENT_EXEC<<8);

    char *const ls_argv[] = {"ls", "/", NULL};
    char *const ls_envp[] = {"PATH=/user/bin:/bin", "TERM=console", NULL};
 ...
 //               if (status >> 16 == PTRACE_EVENT_FORK)
                if (status>>8 == result)
                {
                    ...
                    //PTRACE_EVENT_EXEC kernel version must large than 3.0 then can get the child pid
                    printf("attach %d\n", new_child);
                    is_get_event = 1;
                }
            }
